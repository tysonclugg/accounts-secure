 ...
 * @param {String} options.password The user's password.  We refuse to send this in plain text over the wire.
...
Accounts.createUser = function (options, callback) {
  ...
  if (window.location.protocol != 'https:') {
    callback(new Meteor.Error(400, "Refusing to send password without encryption.")
    return;
  }
 ...
 * @locus Client
 * @param {String} oldPassword The user's current password.  We refuse to send this in plain text over the wire.
 * @param {String} newPassword A new password for the user.  We refuse to send this in plain text over the wire.
 * @param {Function} [callback] Optional callback. Called with no arguments on success, or with a single `Error` argument on failure.
 */
...
Accounts.resetPassword = function(token, newPassword, callback) {
  ...
  Accounts.callLoginMethod({
    ...
    methodArguments: [token, newPassword],
    userCallback: callback});
};
